...
import { applicationLanguagesDropdown } from "../utils/i18n";

const Header = () => {
  const { t, locale, switchLoadLanguage } = useI18nProContext();
  ...
  const onChangeLanguage = (value) => {
    switchLoadLanguage(value, `/i18n/${value}.json`)
  }
        ...
        <div className="content flex max-[500px]:flex-col-reverse flex-row items-center gap-4 ">
          ...
          {isAuthenticated && (
            <span className="text-white text-sm font-medium">
              {t("welcome_message_header")}, {user.name}
            </span>
          )}
          ...
          <nav className="space-x-4 flex items-center">
            ...
            {/* Language Dropdown */}
            <Dropdown value={locale} options={applicationLanguagesDropdown} onChange={onChangeLanguage} className={'hidden sm:block w-36 max-w-xs'} />
          </nav>
        </div>
      </div>
      {/* Language Dropdown */}
      <Dropdown value={locale} options={applicationLanguagesDropdown} onChange={onChangeLanguage} className={'sm:hidden max-w-xs'} />
    </header>
  );
};
